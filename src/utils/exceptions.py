...
class SMNotUsable(QuotientError):
    def __init__(self):
        super().__init__(
            "You need either the `scrims-mod` role or `manage_guild` permissions to use this command."
        )
...
class TMNotUsable(QuotientError):
    def __init__(self):
        super().__init__(
            "You need either the `tourney-mod` role or `manage_guild` permissions to use tourney manager."
        )
